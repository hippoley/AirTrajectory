...
canvas.addEventListener("pointerdown",e=>{const p=canvasPoint(e),hit=hitOpening(p.x,p.y);if(!hit)return;dragOpening=hit;dragMoved=false;selectOpening(hit.id);canvas.setPointerCapture(e.pointerId)});
...
canvas.addEventListener("pointerup",e=>{if(!dragOpening)return;const edited=dragOpening,moved=dragMoved;dragOpening=null;dragMoved=false;if(canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId);if(moved)invalidateTrajectory(edited.id+" moved along wall");else cycleOpening(edited.id)});
canvas.addEventListener("pointercancel",()=>{dragOpening=null;dragMoved=false});
